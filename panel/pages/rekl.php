...
if(isset($_SESSION["$cookiesVar"]) && ($nr == 1 || $nr ==2) ){
	...
	$query1 = "SELECT users.user, users.nick, users.registerData, lvl, lvl_for_points, lvl_for_points_start FROM `uzytkownicy` as users left outer join `dbmod_tablet` as dbmod ON dbmod.nick = users.nick WHERE `rekl` = '$user' AND dbmod.nick = users.nick ORDER by lvl desc";
	//echo $query1;
	$tab1a = array();
...
<?php
	...
	echo '<h1><a href="http://cs-lod.com.pl/system/bannery.php"> Zobacz wszystkie </a></h1>';
